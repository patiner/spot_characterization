...
path = './spot_traces/ec2_prices/us-east-1d/';
filename = 'price_time_r3.xlarge.txt';
dst_path = [path 'r3.xlarge/'];
%mkdir(dst_path);
...
B = [0.1 0.5 1 2 5 10]*0.333; %6 bids
...
for k = 1:length(B)
    ...
    for i = 1:D-window_length-1
        ...
        E_price = find_ES(data,B(k));
        
        predited_LB = predict_LB(cell2mat(duration_dist(k,i+window_length-1)));
            ...
            if LB(j) < predited_LB
                ...
                if j < length(LB) % if we still have more LB
                    ...
                    for n = 1:7
                        ...
                        new_ES = [new_ES find_ES(look_back_data((n-1)*1440+1:n*1440), B(k))];
                    end
                    predited_LB = predict_LB(new_duration);
