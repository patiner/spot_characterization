...
for t = 24*7+1:T
    ...
    ES_OD_c(t) = predict_ES(find_ES(trace_c,p_od));
   
    LB_5OD_c(t) = predict_LB(find_Lb(trace_c,5*p_od));
    ES_5OD_c(t) = predict_ES(find_ES(trace_c,5*p_od));
    
    trace_d = pd((t-1)*60-24*7*60+1:(t-1)*60); %past 7 days
    LB_OD_d(t) = predict_LB(find_Lb(trace_d,p_od));
    ES_OD_d(t) = predict_ES(find_ES(trace_d,p_od));
   
    LB_5OD_d(t) = predict_LB(find_Lb(trace_d,5*p_od));
    ES_5OD_d(t) = predict_ES(find_ES(trace_d,5*p_od));
    
    
end

Lb_new = [LB_OD_c LB_5OD_c LB_OD_d LB_5OD_d ones(T,1)*14400000];
ES_new = [ES_OD_c ES_5OD_c ES_OD_d ES_5OD_d ones(T,1)*p_od];
fdir = ['./' type '/' region '/'];
save([fdir 'Lb_new.mat'],'Lb_new');
save([fdir 'ES_new.mat'],'ES_new');
